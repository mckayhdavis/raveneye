 ...
 */
public class RealityOverlayView extends LayoutSensorView {
    ...
    private void drawPlace(Canvas canvas, Paint paint, Paint textPaint,
        ...
        if (placeLocation != null) {
            ...
            if (distance >= 15000) {
                ...
                factor = 1 - (distance / 18750);
                alpha = (byte) ((0xe2 * (1 - (distance / 20000))));
            }
     ...
     * mHorizontalTranslationCorrection, mVerticalTranslationCorrection); // canvas.translate(mX,
     * mY);
     * 
     * // TODO: mSrc and mDst values are incorrect if mCanvasHeight != // mHeight.
     * 
     * // Draw the generated bitmap. Account for wrap-around at North. if (mX >=
     * -mHorizontalTranslationCorrection) { // canvas.drawBitmap(mBitmap, -mCanvasWidth, 0, null);
     * 
     * mSrc.left = mCanvasWidth - mX; mSrc.top = 0; mSrc.right = mCanvasWidth; mSrc.bottom =
     * mCanvasHeight;
     * 
     * mDst.left = 0; mDst.top = 0; mDst.right = mX; mDst.bottom = mHeight;
     * 
     * canvas.drawBitmap(mBitmap, mSrc, mDst, null); } else if (mX <= ((mWidth +
     * mHorizontalTranslationCorrection) - mCanvasWidth)) { // canvas.drawBitmap(mBitmap,
     * mCanvasWidth, 0, null);
     * 
     * mSrc.left = mCanvasWidth - mX; mSrc.top = 0; mSrc.right = mCanvasWidth; mSrc.bottom =
     * mCanvasHeight;
     ...
     * mDst.left = 0; mDst.top = 0; mDst.right = mX; mDst.bottom = mHeight;
     * 
     * canvas.drawBitmap(mBitmap, mSrc, mDst, null); } else { mSrc.left = mCanvasWidth + mX;
     * mSrc.top = 0; mSrc.right = mX + mWidth; mSrc.bottom = mCanvasHeight;
     * 
     * mDst.left = 0; if (mY >= 0) { mDst.top = mY; mDst.bottom = mHeight; } else { mDst.top = 0;
     * mDst.bottom = mHeight + mY; } mDst.right = mWidth; }
     * 
     * canvas.drawBitmap(mBitmap, mSrc, mDst, null); // .drawBitmap(mBitmap, 0, 0, null);
     */
