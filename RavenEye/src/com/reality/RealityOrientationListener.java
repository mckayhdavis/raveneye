 ...
 * noise-free sensor values. This is useful for situations in which sensor values play some part in
 * the visual representation and transformation of UI elements on screen.
 * 
...
public class RealityOrientationListener implements SensorEventListener,
    ...
    public void filterValues(final float[] values) {
        ...
        /*
         * Account for azimuth rotations around SOUTH. Azimuth values range is (-180,180].
         * 
         * Only account for wrap-around at SOUTH. Don't use corrections at the NORTH azimuth value
         * (ie. around 0 degrees).
         */
        ...
        if (values[0] < -360) {
            ...
            // TODO: Temporary fix (Sometimes the oldValues[0] is out of bounds).
            oldValues[0] = 0;
        } else if (values[0] < 0) {
            values[0] += 360;
    ...
    public void onLocationChanged(Location location) {
        /*
         * We can safely assume that we only need one declination angle for the duration of the
         * application runtime. Most likely, the user will not travel such great distances in
         * reality mode.
         */
        if (!mHasDeclination) {
            float altimeter = 0;
