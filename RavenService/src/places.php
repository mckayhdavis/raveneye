...
switch($content) {
	case 'basic':
		$query = "SELECT ID, Places.Name as Name, Street, City, Provinces.Code as PCode, PostalCode, Country, BuildingCodes.Code as BCode, Latitude, Longitude, FileName, Caption, ReviewCount FROM (((((((Places NATURAL JOIN Cities) NATURAL JOIN Provinces) NATURAL JOIN Countries) LEFT JOIN BuildingCodes ON Places.ID = BuildingCodes.PlaceID) LEFT OUTER JOIN PlaceImage ON Places.ID = PlaceImage.PlaceID) LEFT OUTER JOIN Locations ON Places.ID = Locations.PlaceID) LEFT OUTER JOIN ((SELECT PlaceID, count(Reviews.PlaceID) as ReviewCount FROM Reviews) as ReviewsCount) ON Places.ID = ReviewsCount.PlaceID) WHERE $placeCriteria ORDER BY Places.Name ASC LIMIT $offset, $number_of_posts";
		break;
	case 'extended':
	default:
		$query = "SELECT ID, Places.Name as Name, Street, City, Provinces.Code as PCode, PostalCode, Country, Description, BuildingCodes.Code as BCode, Latitude, Longitude, FileName, Caption, ReviewCount FROM (((((((Places NATURAL JOIN Cities) NATURAL JOIN Provinces) NATURAL JOIN Countries) LEFT JOIN BuildingCodes ON Places.ID = BuildingCodes.PlaceID) LEFT OUTER JOIN PlaceImage ON Places.ID = PlaceImage.PlaceID) LEFT OUTER JOIN Locations ON Places.ID = Locations.PlaceID) LEFT OUTER JOIN ((SELECT PlaceID, count(Reviews.PlaceID) as ReviewCount FROM Reviews) as ReviewsCount) ON Places.ID = ReviewsCount.PlaceID) WHERE $placeCriteria ORDER BY Places.Name ASC LIMIT $offset, $number_of_posts";
}
